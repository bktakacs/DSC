...
the member name. */
SELECT DISTINCT (m.surname || ', ' || m.firstname) as name,
    CASE WHEN b.facid = 0 THEN 'Tennis Court 1'
            WHEN b.facid = 1 THEN 'Tennis Court 2'
            ELSE 'NA' END AS courtname
FROM Bookings AS b
LEFT JOIN Members AS m
ON b.memid = m.memid
WHERE courtname IN ('Tennis Court 1', 'Tennis Court 2')
AND name NOT IN ('GUEST, GUEST')
ORDER BY name ASC;
...
SELECT f.name, (m.surname || ', ' || m.firstname) as member,
...
SELECT m.surname || ', ' || m.firstname as member, sub.facility,
    sub.cost
FROM Members AS m,
    (
    SELECT b.memid, f.name AS facility,
...
WHERE sub.cost > 30 AND m.memid = sub.memid
ORDER BY cost DESC;
